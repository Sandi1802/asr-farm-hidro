...
function renderDailySchedule(dateStr) {
    ...
    // Stage config
    const stageMap = {
        semai:   { icon:'ph-seedling',  color:'#16a34a', bg:'#dcfce7', label:'Fase Penyemaian',  emoji:'ðŸŒ±' },
        tanam:   { icon:'ph-plant',     color:'#2563eb', bg:'#dbeafe', label:'Fase Penanaman',   emoji:'ðŸª´' },
        remaja:  { icon:'ph-leaf',      color:'#9333ea', bg:'#f3e8ff', label:'Fase Remaja',       emoji:'ðŸŒ¿' },
        harvest: { icon:'ph-basket',    color:'#e11d48', bg:'#ffe4e6', label:'Jadwal Panen',      emoji:'ðŸŒ¾' },
        planting:{ icon:'ph-plant',     color:'#16a34a', bg:'#dcfce7', label:'Penanaman',         emoji:'ðŸŒ±' },
        custom:  { icon:'ph-bookmark',  color:'#0891b2', bg:'#ecfeff', label:'Kegiatan',          emoji:'ðŸ“Œ' },
    };
    ...
    evList.forEach(ev => {
        ...
        if (ev.type === 'harvest') {
            ...
            subtitle = `${ev.location} Â· Umur: ${ev.days_old} hari`;
            ...
            if (ev.harvested_by)  subtitle += ` Â· <i class="ph ph-user" style="font-size:0.7rem;"></i> ${ev.harvested_by}`;
        } else if (ev.type === 'semai') {
            title    = `<span style="font-weight:700;">${cfg.emoji} Penyemaian ${ev.plant_name}</span>`;
            subtitle = `${ev.location} Â· Hari ke-${ev.stage_day}`;
...
function initCharts() {
    ...
    // â”€â”€ 1. Tanaman Paling Sering Ditanam â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ 2. Tanaman Paling Sering Dipanen â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ 3. Perputaran / Occupancy per Greenhouse â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ 3B. Distribusi Green House (Pie) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ 3C. Tren Produksi (4 Minggu Terakhir)
    ...
    // â”€â”€ 3D. Top 8 Tanaman Paling Sering Ditanam
    const plantedCtx = document.getElementById('chartMostPlanted');
    if (plantedCtx) {
        new Chart(plantedCtx.getContext('2d'), {
            type: 'bar',
            data: {
                labels: {!! json_encode($mostPlantedLabels ?? []) !!},
                datasets: [{
                    label: 'Jumlah Ditanam',
                    data: {!! json_encode($mostPlantedValues ?? []) !!},
    ...
    // â”€â”€ 3E. Top 8 Tanaman Paling Sering Dipanen
    const harvestedCtx = document.getElementById('chartMostHarvested');
    if (harvestedCtx) {
        new Chart(harvestedCtx.getContext('2d'), {
            type: 'bar',
            data: {
                labels: {!! json_encode($mostHarvestedLabels ?? []) !!},
                datasets: [{
                    label: 'Jumlah Dipanen',
                    data: {!! json_encode($mostHarvestedValues ?? []) !!},
    ...
    // â”€â”€ 3F. Tingkat Occupancy Tiap GH (Perputaran)
    const rotationCtx = document.getElementById('chartRotation');
    if (rotationCtx) {
        const rotData = {!! json_encode($rotationData ?? []) !!};
        if (rotData && rotData.labels) {
            new Chart(rotationCtx.getContext('2d'), {
                type: 'bar',
                data: {
                    labels: rotData.labels,
                    datasets: [{
...
// â”€â”€ Chart UI interactions & API Updates â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
