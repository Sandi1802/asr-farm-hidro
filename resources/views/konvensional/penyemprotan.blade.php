...
<div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 2rem;">
    <div>
        <h1 style="margin: 0; font-size: 1.5rem; color: var(--text-color);">Riwayat Penyemprotan</h1>
        <p style="margin: 0.25rem 0 0; color: var(--text-muted);">Jadwal dan catatan penyemprotan hama/penyakit</p>
    </div>
    ...
    <table class="table datatable" style="width: 100%; border-collapse: collapse; text-align: left;">
        <thead style="background: #f8fafc; border-bottom: 1px solid var(--border-color);">
            <tr>
                ...
                <th style="padding: 1rem; color: var(--text-muted); font-weight: 500; font-size: 0.875rem;">Nama Obat/Pestisida</th>
                <th style="padding: 1rem; color: var(--text-muted); font-weight: 500; font-size: 0.875rem;">Dosis</th>
                <th style="padding: 1rem; color: var(--text-muted); font-weight: 500; font-size: 0.875rem;">Tujuan/Alasan</th>
                <th style="padding: 1rem; color: var(--text-muted); font-weight: 500; font-size: 0.875rem; text-align: right;">Aksi</th>
            </tr>
        </thead>
        <tbody>
            @foreach($penyemprotan as $p)
            <tr style="border-bottom: 1px solid var(--border-color);">
                ...
                <td style="padding: 1rem; text-align: right; display: flex; justify-content: flex-end; gap: 0.5rem;">
                    <form action="{{ route('konvensional.penyemprotan.destroy', $p->id) }}" method="POST" onsubmit="return confirm('Hapus riwayat ini?');" style="margin: 0; display: inline;">
                        @csrf
                        @method('DELETE')
...
<div id="modalTambah" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center;">
    <div style="background: white; border-radius: 12px; width: 100%; max-width: 500px; padding: 1.5rem; margin: 1rem;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <h3 style="margin: 0;">Catat Penyemprotan Baru</h3>
        ...
        <form action="{{ route('konvensional.penyemprotan') }}" method="POST">
            @csrf
            ...
            <div style="margin-bottom: 1rem;">
                <label style="display: block; margin-bottom: 0.5rem; font-weight: 500; color: var(--text-color);">Nama Obat/Pestisida *</label>
                <input type="text" name="nama_obat" required style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px;" placeholder="Contoh: Furadan">
            </div>
            
            <div style="margin-bottom: 1rem; display: flex; gap: 1rem;">
                <div style="flex: 1;">
                    <label style="display: block; margin-bottom: 0.5rem; font-weight: 500; color: var(--text-color);">Dosis *</label>
                    <input type="text" name="dosis" required style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px;" placeholder="Contoh: 2 ml/liter">
                </div>
                <div style="flex: 1;">
            ...
            <div style="margin-bottom: 1rem;">
                <label style="display: block; margin-bottom: 0.5rem; font-weight: 500; color: var(--text-color);">Tujuan/Alasan Penyakit</label>
                <input type="text" name="alasan" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 6px;" placeholder="Contoh: Serangan hama kutu putih (Opsional)">
            </div>
