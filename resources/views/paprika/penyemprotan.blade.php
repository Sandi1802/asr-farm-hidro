...
<div class="dashboard-header">
    <div style="display: flex; align-items: center; justify-content: space-between;">
        <div>
            <h1 style="font-size: 1.5rem; color: var(--text-main); font-weight: 600; margin-bottom: 0.5rem;">Penyemprotan Paprika</h1>
            <p style="color: var(--text-muted); font-size: 0.95rem;">Jadwal dan riwayat penyemprotan tanaman paprika.</p>
        </div>
        <button type="button" class="btn btn-primary" onclick="openModal('addPenyemprotanModal')" style="background: var(--asr-green); color: white; border: none; padding: 0.5rem 1rem; border-radius: 8px; cursor: pointer; font-weight: 500; display: inline-flex; align-items: center; gap: 0.5rem;">
...
<div class="card" style="background: var(--card-bg); border-radius: 12px; border: 1px solid var(--border-color); padding: 1.5rem; margin-top: 1.5rem;">
    <div class="table-responsive" style="overflow-x: auto;">
        <table class="table datatable" id="penyemprotanTable" style="width: 100%; border-collapse: collapse;">
            <thead>
                <tr>
                    <th class="dt-no" style="width: 50px;">NO</th>
                    <th>TANGGAL</th>
                    <th>GREENHOUSE</th>
                    <th>JENIS OBAT/PESTISIDA</th>
                    <th>DOSIS</th>
                    <th>PEKERJA</th>
                </tr>
            </thead>
            <tbody>
                @foreach($sprayings as $index => $spray)
                <tr style="border-bottom: 1px solid var(--border-color);">
                    <td style="padding: 1rem; text-align: center; color: var(--text-muted);">{{ $index + 1 }}</td>
                    <td style="padding: 1rem; color: var(--text-main); font-weight: 500;">{{ \Carbon\Carbon::parse($spray->date)->format('d M Y') }}</td>
                    <td style="padding: 1rem; color: var(--text-muted);">{{ $spray->greenhouse ? $spray->greenhouse->name : '-' }}</td>
                    <td style="padding: 1rem; color: var(--text-muted);">{{ $spray->pesticide_name }}</td>
                    <td style="padding: 1rem; color: var(--text-muted);">{{ $spray->dose }}</td>
                    <td style="padding: 1rem; color: var(--text-muted);">{{ $spray->worker_name }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
...
<!-- Modal Tambah Penyemprotan -->
<div id="addPenyemprotanModal" class="modal" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; overflow: auto; background-color: rgba(0,0,0,0.5);">
    <div class="modal-content" style="background-color: var(--card-bg); margin: 10% auto; padding: 2rem; border: 1px solid var(--border-color); width: 100%; max-width: 500px; border-radius: 12px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <h2 style="font-size: 1.25rem; font-weight: 600; color: var(--text-main);">Tambah Jadwal Penyemprotan</h2>
            <span onclick="closeModal('addPenyemprotanModal')" style="color: var(--text-muted); font-size: 1.5rem; font-weight: bold; cursor: pointer;">&times;</span>
        </div>
        <form action="/paprika/penyemprotan" method="POST">
            @csrf
            ...
            <div style="margin-bottom: 1rem;">
                <label style="display: block; margin-bottom: 0.5rem; color: var(--text-main); font-weight: 500;">Jenis Obat/Pestisida</label>
                <input type="text" name="pesticide_name" required placeholder="Contoh: Insektisida XYZ" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 8px; background: var(--card-bg); color: var(--text-main);">
            </div>
            <div style="margin-bottom: 1rem;">
                <label style="display: block; margin-bottom: 0.5rem; color: var(--text-main); font-weight: 500;">Dosis</label>
                <input type="text" name="dose" required placeholder="Contoh: 10ml / Liter" style="width: 100%; padding: 0.75rem; border: 1px solid var(--border-color); border-radius: 8px; background: var(--card-bg); color: var(--text-main);">
            </div>
            ...
            <div style="display: flex; justify-content: flex-end; gap: 1rem;">
                <button type="button" onclick="closeModal('addPenyemprotanModal')" style="padding: 0.5rem 1rem; border: 1px solid var(--border-color); background: transparent; color: var(--text-main); border-radius: 8px; cursor: pointer;">Batal</button>
